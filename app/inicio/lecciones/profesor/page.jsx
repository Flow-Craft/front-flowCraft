...
import { getInscripcionesALecciones, getleccionesAsignadas, iniciarLeccionAdmin } from '@/app/utils/actions';
...
function Page() {
  ...
  const obtenerLecciones = async() =>{
    ...
    const leccionesToShow = result && result.map((lecc)=>({
      id:lecc.id,
      nombre:lecc.nombre,
      categoria:lecc.categoria.nombre,
      disciplina:lecc.disciplina.nombre,
      dias:<ul>{lecc.dias.map((dia,i)=><li key={i}>{dia}</li>)}</ul>,
      horarios:lecc.horarios[0],
      lugar:lecc.lugar,
      acciones:ActionTab(
        result.find((disc) => disc.id === lecc.id),
      )
    }))
  ...
  const handleStartLesson = async(id) =>{
    ...
    setAlumnos(result && result.filter((alumno)=>alumno.fechaBaja === null).map((alumno)=>({value:alumno.usuario.id, label: `${alumno.usuario.dni} - ${alumno.usuario.apellido} ${alumno.usuario.nombre}`})))
  ...
  const iniciarLeccion = async() =>{
    try {
      ...
      setAlumnosPresentes([]);
      router.push(`/inicio/lecciones/profesor/estadisticas/${leccionSeleccionada.id}`)
    } catch (error) {
  ...
  const ActionTab = (leccion) => {
    ...
    const estaEnRango = minutosActuales >= minutosInicio && minutosActuales <= minutosFin;
    const objetoConIdMasGrande = leccion.leccionHistoriales.reduce((max, obj) => 
        ...
        <div className="flex flex-row gap-4">
          {mostrarPlay(estaEnRango,esDiaDeLeccion,esFinalizadoHoy) ? (
            <Tooltip label="Empezar leccion">
              <PlayIcon
                onClick={() => {
                  if (objetoConIdMasGrande.leccionEstado.nombreEstado === "ClaseIniciada") {
                    router.push(`/inicio/lecciones/profesor/estadisticas/${leccion.id}`);
                    return;
                  }
            ...
            <Tooltip label="Editar Estadisticas">
              <ChartBarIcon
                onClick={() => {
                  router.push(`/inicio/lecciones/profesor/estadisticas/${leccion.id}?esEdicion=true`);
                  return;
                }}
                className="w-[50px] cursor-pointer text-slate-500"
              />
