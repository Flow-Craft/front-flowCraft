...
function Page() {
  ...
  const getAllFilters = async () => {
    ...
    setUsuariosToShow(userOptions);
  };

  const eliminarDelEquipo = (id) => {
    setUsuariosSeleccionados((current) => {
      const newCurrent = current.filter((user) => user.id !== id);
      return newCurrent;
    });
    const userAAgregar = usuarios.find((user) => user.id === id);
    const nuevaOpcion = {
    ...
    const nuevoUsuario = {
      ...
      acciones: (
        <XCircleIcon
          ...
          onClick={() => {
            eliminarDelEquipo(userSelected.id);
          }}
        />
      ),
    };
