...
export const EquiposTab = () => {
  ...
  const handleFormEquipo = async (e) => {
    try {
      ...
      toast.success('Equipo Estado creado con exito');
      equipoToTab();
  ...
  const editarEquipo = async (e) => {
    try {
      ...
      toast.success('Equipo editado con exito');
      equipoToTab();
      setOpenCreateEquipo(false);
      setEquipoToEdit({});
    } catch (error) {
  ...
  const equipoToTab = async () => {
    try {
      const result = await getEquipoEstadoAdmin();
      setEquipo(result);
      const equipo =
        result &&
        result.map((dis) => {
  ...
  return (
    <div className="mt-7">
      ...
      <Toaster />
      <FlowModal
        title={`Eliminar Equipo ${equipoToDelte?.nombreEstado}`}
        modalBody={
          <div>
            ¿Esta seguro que desea eliminar esta equipo Estado?{' '}
      ...
      />
      <FlowModal
        title={`${equipoToEdit?.id ? 'Editar equipo' : 'Crear equipo'}`}
        modalBody={
          <div>
