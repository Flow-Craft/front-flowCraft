...
export const InstalacionTab = () => {
  ...
  const equipoToTab = async () => {
    try {
      const result = await getInstalacionEstadoAdmin();
      setEquipo(result);
      const equipo =
        result &&
        result.map((dis) => {
  ...
  return (
    <div className="mt-7">
      ...
      <FlowModal
        ...
        modalBody={
          <div>
            ¿Esta seguro que desea eliminar esta instalacion Estado?{' '}
      ...
      />
      <FlowModal
        title={`${equipoToEdit?.id ? 'Editar instalacion estado' : 'Crear instalacion estado'}`}
        modalBody={
          <div>
