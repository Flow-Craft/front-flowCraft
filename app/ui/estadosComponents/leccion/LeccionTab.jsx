...
import {
  eliminarLeccionEstadoAdmin,
  crearLeccionEstadoAdmin,
  editarLeccionEstadoAdmin,
  getLeccionEstadoAdmin,
...
export const LeccionTab = () => {
  ...
  const deleteCategoria = async () => {
    try {
      await eliminarLeccionEstadoAdmin(equipoToDelte.id);
      toast.success('Leccion Estado eliminada con éxito');
      equipoToTab();
    } catch (error) {
  ...
  const handleFormEquipo = async (e) => {
    try {
      ...
      const result = await crearLeccionEstadoAdmin(equipo, setErrors);
      if (result?.error) {
        setErrors(result.errors);
        return;
      }
      toast.success('Leccion Estado creada con éxito');
      equipoToTab();
  ...
  const editarEquipo = async (e) => {
    try {
      ...
      await editarLeccionEstadoAdmin(categoria);
      toast.success('Leccion Estado editada con éxito');
      equipoToTab();
      setOpenCreateEquipo(false);
      setEquipoToEdit({});
    } catch (error) {
  ...
  const equipoToTab = async () => {
    try {
      const result = await getLeccionEstadoAdmin();
      setEquipo(result);
      const equipo =
        result &&
        result.map((dis) => {
  ...
  return (
    <div className="mt-7">
      ...
      />
      <FlowModal
        title={`${equipoToEdit?.id ? 'Editar leccion estado' : 'Crear leccion estado'}`}
        modalBody={
          <div>
